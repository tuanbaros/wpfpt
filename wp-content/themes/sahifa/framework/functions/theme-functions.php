...
/*-----------------------------------------------------------------------------------*/
function tie_last_posts_cat_timeline($numberOfPosts = 5 , $cats = 1){
	global $post;
	$orig_post = $post;
...
/*-----------------------------------------------------------------------------------*/
function tie_most_viewed($pop_posts = 5 , $thumb = true){
	global $post;
	$orig_post = $post;
	
	$popularposts = new WP_Query(array('posts_per_page' => $pop_posts, 'orderby' => 'meta_value_num' ,  'meta_key' => 'tie_views', 'post_status' => 'publish', 'no_found_rows' => 1, 'ignore_sticky_posts' => 1  ));
	while ( $popularposts->have_posts() ) : $popularposts->the_post()?>
			<li <?php tie_post_class(); ?>>
			<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $thumb ) : ?>			
				<div class="post-thumbnail">
...
function tie_best_reviews_posts($pop_posts = 5 , $thumb = true){
	...
	$cat_query1 = new WP_Query(array('posts_per_page' => $pop_posts, 'orderby' => 'meta_value_num' ,  'meta_key' => 'taq_review_score', 'post_status' => 'publish', 'no_found_rows' => 1 ));
...
function tie_login_form( $login_only  = 0 ) {
	...
	if ( $user_ID ) : ?>
		...
		<div id="user-login">
			...
			<ul>
				<li><a href="<?php echo home_url() ?>/wp-admin/"><?php _eti( 'Dashboard' ) ?> </a></li>
				<li><a href="<?php echo home_url() ?>/wp-admin/profile.php"><?php _eti( 'Your Profile' ) ?> </a></li>
				<li><a href="<?php echo wp_logout_url(); ?>"><?php _eti( 'Logout' ) ?> </a></li>
			</ul>
			<div class="clear"></div>
		</div>
		<?php endif; ?>
	<?php else: ?>
		<div id="login-form">
			...
			<ul class="login-links">
				...
				<li><a href="<?php echo home_url() ?>/wp-login.php?action=lostpassword"><?php _eti( 'Lost your password?' ) ?></a></li>
			</ul>
		</div>
	<?php endif;
}
...
/*-----------------------------------------------------------------------------------*/
# WPML LANGUAGES HOME PAGES
/*-----------------------------------------------------------------------------------
add_filter('nav_menu_css_class', 'tie_add_current_menu_item_class',10,2);
function tie_add_current_menu_item_class($classes,$item){
	$link = site_url().$_SERVER['REQUEST_URI'];
	if( !empty( $item->url ) && strpos($link, $item->url) !== false && $item->type != 'custom'){
